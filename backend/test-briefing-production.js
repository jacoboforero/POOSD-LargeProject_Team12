...
const BASE_URL = 'http://129.212.183.227:3001';
...
async function testFullBriefingFlow() {
  console.log('🧪 END-TO-END BRIEFING GENERATION TEST (PRODUCTION)');
  ...
  try {
    ...
    // Step 2: Ask user for OTP from server logs
    console.log('🔐 STEP 2: Please check the server logs for the OTP code.');
    console.log('   You can view logs with: pm2 logs news-briefing-api --lines 50');
    console.log('   Look for a log entry with your email and the OTP code.\n');
    
    const otp = await askQuestion('Enter the 6-digit OTP code: ');
